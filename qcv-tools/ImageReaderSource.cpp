...
inline char ImageReaderSource::convertPixel(char const* pixel_) const {
  ...
  if (comps == 1 || comps == 2) {
    ...
    throw qcv::IllegalArgumentException("Unexpected image depth");
  }
}
  ...
  if (extension == "png") {
    ...
    { unsigned w, h;
      ...
      if (error) {
        ...
        throw qcv::IllegalArgumentException(msg.str().c_str());
      }
      width = w;
      height = h;
    }
    ...
    comps = 4;
    image = qcv::ArrayRef<char>(4 * width * height);
    ...
    image = qcv::ArrayRef<char>(buffer, 4 * width * height);
    free(buffer);
  }
  if (!image) {
    ostringstream msg;
...
qcv::ArrayRef<char> ImageReaderSource::getRow(int y, qcv::ArrayRef<char> row) const {
  const char* pixelRow = &image[0] + y * getWidth() * 4;
  if (!row) {
    row = qcv::ArrayRef<char>(getWidth());
  }
...
qcv::ArrayRef<char> ImageReaderSource::getMatrix() const {
  const char* p = &image[0];
  qcv::ArrayRef<char> matrix(getWidth() * getHeight());
  char* m = &matrix[0];
