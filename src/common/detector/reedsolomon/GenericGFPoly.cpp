...
Ref<GenericGFPoly> GenericGFPoly::addOrSubtract(Ref<qcv::GenericGFPoly> other) {
...
Ref<GenericGFPoly> GenericGFPoly::multiply(Ref<qcv::GenericGFPoly> other) {
  if (!(&field_ == &other->field_)) {
    throw IllegalArgumentException("GenericGFPolys do not have same GenericGF field");
  }
